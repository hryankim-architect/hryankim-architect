...
| [`dmoi-brca-poc`](https://github.com/hryankim-architect/dmoi-brca-poc) | Hypothesis-conditioned multi-omics (RNA + HM450 methylation) on TCGA-BRCA LumA-vs-LumB, with cross-cohort external validation on METABRIC, per-patient interpretability rolled up to the full 50-set MSigDB Hallmark catalog, and a learnable pathway-pole attention experiment that documents an honest architectural failure mode | **v0.7 Phase A** · TCGA held-out test **AUROC 0.968** (n=84, v0.6 canonical) · **METABRIC external AUROC 0.909** (n=1,175, RNA-only) · Per-patient **Integrated Gradients** picked FOXC1/BCL2 for the LumA pole and RANBP1/NBN/ZW10/POLA2 for the LumB pole (ESR1/PGR correctly absent), cross-cohort gene-level Jaccard top-10 = 0.667 for both poles; v0.5 / v0.6 rolled the per-gene IG up to MSigDB Hallmark and the LumA pole loaded ER ~300× harder than cell-cycle while the LumB pole loaded cell-cycle ~45× harder than ER, finding survives the full 50-set catalog widening on both cohorts (top-3 of 50 identical to top-3 of 5); **v0.7 Phase A swapped the hand-picked pole masks for a learnable softmax-over-50-pathways attention and documented a self-reinforcing softmax-attention collapse failure mode under standardized inputs — top weights at 1/50 uniform, 0 of 3 v0.6 top pathways made the learned top-3, AUROC roughly neutral. Honest negative shipped + Phase B retry with raw inputs + warmer init planned.** |
