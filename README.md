...
| [`dmoi-brca-poc`](https://github.com/hryankim-architect/dmoi-brca-poc) | Hypothesis-conditioned multi-omics (RNA + HM450 methylation) on TCGA-BRCA, with cross-cohort external validation, full-catalog Hallmark interpretability rollup, a 3-variant falsifiable learnable-pathway-attention experiment, and cross-task generalization to Luminal-vs-Basal | **v0.9** · LumA-vs-LumB TCGA test **AUROC 0.968** (n=84) + METABRIC **0.909** (n=1,175); per-patient IG picked FOXC1/BCL2 / RANBP1/NBN/ZW10/POLA2 cross-cohort (Jaccard 0.667); v0.5/v0.6 Hallmark rollup showed LumA pole loads ER ~300× cell-cycle and LumB loads cell-cycle ~45× ER on both cohorts, finding survives full 50-set widening; **v0.7+v0.8 ran 3 progressively richer learnable pathway-attention variants and all 3 confirmed gene-level commitment is the right architectural level (Phase A collapse → Phase B wrong-basin scalar → Variant C wrong-basin vector with 17× more params); v0.9 transferred the same v0.6 architecture to a different classification axis (Luminal vs Basal, n=502 dual-modality) with no model changes and reached TCGA test AUROC 1.000 (bacc 0.972) and 8/8 expected priors in the per-pole IG top-5 (Luminal: ER_EARLY/ER_LATE/ANDROGEN; Basal: MYC/G2M/EMT/E2F/MYC_V2) -- framework reusability empirically confirmed end-to-end.** |
