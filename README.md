...
| [`sc-tumor-annotator`](https://github.com/hryankim-architect/sc-tumor-annotator) | Tree-based hierarchical tumor scRNA-seq annotation + expression-based CNV inference (chromosome-length-normalized malignant-cell score) + cancer subtype/grade prediction; clean-room, synthetic-data POC built from public methods only | **v0.1** · 4-stage annotator (compartment → stromal type → malignant call → subtype + grade) with the CNV score fed as a malignant-call feature · 5-fold CV + independent-cohort harness vs a kNN reference-mapping baseline · on the hardest axis (grade) the trainable tree beats the CNV-blind baseline, macro-F1 **0.938 CV / 0.910 independent vs 0.874** · deterministic, CPU-only, 14 tests, ruff + English-only CI |
