...
| [`dmoi-brca-poc`](https://github.com/hryankim-architect/dmoi-brca-poc) | Hypothesis-conditioned multi-omics (RNA + HM450 methylation) on TCGA-BRCA LumA-vs-LumB, with cross-cohort external validation on METABRIC, per-patient interpretability rolled up to the full 50-set MSigDB Hallmark catalog, and a two-phase learnable pathway-pole attention experiment that documents an architectural failure mode end to end | **v0.7.1** · TCGA held-out test **AUROC 0.968** (n=84, v0.6 canonical) · **METABRIC external AUROC 0.909** (n=1,175, RNA-only) · Per-patient **Integrated Gradients** picked FOXC1/BCL2 for the LumA pole and RANBP1/NBN/ZW10/POLA2 for the LumB pole (ESR1/PGR correctly absent), cross-cohort gene-level Jaccard top-10 = 0.667 for both poles; v0.5 / v0.6 rolled the per-gene IG up to MSigDB Hallmark and the LumA pole loaded ER ~300× harder than cell-cycle while the LumB pole loaded cell-cycle ~45× harder than ER, finding survives the full 50-set catalog widening on both cohorts (top-3 of 50 identical to top-3 of 5); **v0.7 swapped the hand-picked pole masks for a learnable softmax-over-50-pathways attention and ran in two phases: Phase A documented a self-reinforcing softmax-attention collapse under standardized inputs (top weights at 1/50 uniform); Phase B fixed the collapse with raw inputs + warmer init (LumA top weight 3.4× uniform — clear learning) but the model converged on a different basin than the v0.6 IG-derived ranking (0/3 top-3 overlap, AUROC dropped). Diagnosis: scalar-per-pole softmax dot-product captures pathway magnitude variance, not direction signal. v0.8 attempts Variant C (scalar → vector pole feature) to test whether a richer head interface lets the model learn direction signals.** |
