...
| [`dmoi-brca-poc`](https://github.com/hryankim-architect/dmoi-brca-poc) | Hypothesis-conditioned multi-omics (RNA + HM450 methylation) on TCGA-BRCA LumA-vs-LumB, with cross-cohort external validation on METABRIC + per-patient interpretability validated cross-cohort | **v0.4** · TCGA held-out test **AUROC 0.968** (n=84) · **METABRIC external AUROC 0.909** (n=1,175, RNA-only) · Honest negative on architecture lifting AUROC; calibration is the v0.1 win (ECE 0.138 → 0.077 honest); cohort-specific T finding (T_TCGA=0.634 vs T_METABRIC=0.934); per-patient **Integrated Gradients** attribution showed the LumA pole learned inverse-basal-marker discrimination (FOXC1/KRT15) + BCL2, the LumB pole learned cell-cycle markers (RANBP1/NBN/ZW10/POLA2), and ESR1/PGR are correctly absent — **v0.4 validated the biology cross-cohort on METABRIC (Jaccard top-10 = 0.667 for both poles), with METABRIC surfacing MORE canonical proliferation genes (CKS1B/DBF4/NDC80/DSCC1) than TCGA alone** |
