...
## Capability-portrait portfolio (8 public repos)
...
Across the portfolio the seven capability portraits span the axes a clinical-bioinformatics principal is expected to own end to end: pipeline orchestration (`healthomics-lab-orchestrator`), machine learning on operational QC telemetry (`multiqc-foundation-gate`), clinical genomics with survival modeling (`tp53-aml-hrd-severity`), multimodal tumor-microenvironment inference (`hnscc-time-multimodal`), hypothesis-conditioned multi-omics interpretability (`dmoi-brca-poc`, the flagship), single-cell tumor annotation with copy-number inference (`sc-tumor-annotator`), and regulated CDx reporting with 21 CFR Part 11-style electronic signatures (`cdx-clinical-reporter`) — all sharing the eighth repo's scaffold. No single project carries the portfolio; the span does.
