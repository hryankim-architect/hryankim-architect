...
## Capability-portrait portfolio (10 public repos)
...
| [`cdx-clinical-reporter`](https://github.com/hryankim-architect/cdx-clinical-reporter) | Vendor-agnostic companion-diagnostic (CDx) integration shim + 21 CFR Part 11-style signed clinical reporting; clean-room, synthetic-data POC of the regulated-reporting axis | **v0.1** · normalizes Roche/Thermo/Guardant-shaped assay outputs into one canonical finding model · content-hashed clinical report with therapy implications · **electronic-signature ledger** (hash-chained, bound to the content hash) with authored+approved sign-offs and tamper re-verification · deterministic, 18 tests, ruff + English-only CI |
| [`spatial-niche-mapper`](https://github.com/hryankim-architect/spatial-niche-mapper) | Spatial-transcriptomics spot deconvolution + tissue-niche detection; clean-room, synthetic-data POC of the spatial axis | **v0.1** · NNLS spot deconvolution vs a single-cell reference (mean per-cell-type correlation **0.918**, MAE 0.060) · spatial k-NN neighbourhood-composition niche detection (**ARI 0.921** vs ground-truth tumor-core / immune-margin / stroma regions) · deterministic, 14 tests, ruff + English-only CI |
| [`perturb-response-poc`](https://github.com/hryankim-architect/perturb-response-poc) | Single-cell perturbation-response (Perturb-seq style): effect recovery + held-out response prediction; clean-room, synthetic-data POC | **v0.1** · differential-expression program recovery (**F1 0.989**) + effect-size ranking (Spearman 0.986) · ridge feature→response with **leave-out-perturbation** CV (held-out corr **0.60** vs 0.04 mean baseline, lift +0.55) · deterministic, 13 tests, ruff + English-only CI |
...
Across the portfolio the eight capability portraits span the axes a clinical-bioinformatics principal is expected to own end to end: pipeline orchestration (`healthomics-lab-orchestrator`), machine learning on operational QC telemetry (`multiqc-foundation-gate`), clinical genomics with survival modeling (`tp53-aml-hrd-severity`), multimodal tumor-microenvironment inference (`hnscc-time-multimodal`), hypothesis-conditioned multi-omics interpretability (`dmoi-brca-poc`, the flagship), single-cell tumor annotation with copy-number inference (`sc-tumor-annotator`), regulated CDx reporting with 21 CFR Part 11-style electronic signatures (`cdx-clinical-reporter`), spatial-transcriptomics deconvolution + niche mapping (`spatial-niche-mapper`), and single-cell perturbation-response prediction (`perturb-response-poc`) — all sharing the tenth repo's scaffold. No single project carries the portfolio; the span does.
